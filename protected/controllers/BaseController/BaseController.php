...
class BaseController{
	...
	{ 
...

		{
			...
			if($this->db->row("select * from `currency` where id='{$_POST['currency']}'"))
...

	{
        ...
        {	
            ...
            $styles=array_merge(array('style2.css', 'fancy.css', 'sticker.css', 'menu.css'), $param['styles']);
            ...
            $data['menu'] = $view->Render('top_menu.phtml',	array_merge(array('menu'=>$menu, 'translate'=>$data['translate'])));

			$data['menu_f'] = $view->Render('menu.phtml',	array_merge(array('menu'=>$menu, 'translate'=>$data['translate'])));
...
//-----------------функция для вывода каталога SELECT------------------------------------------------------
...

	{
		...
		if(!$page)$page = $this->db->row("SELECT ".$index." FROM `menu` tb1
	...
	public function Lentahtml(){

		$view = new View($this->registry);

		 

		$vars['t1'] = $this->getBlock(1);

		$vars['t2'] = $this->getBlock(2);

		$vars['t3'] = $this->getBlock(3);

		$vars['t4'] = $this->getBlock(4);

		

		return  $view->Render('lentahtml.phtml', $vars);
