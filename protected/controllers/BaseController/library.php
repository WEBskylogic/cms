...
foreach ($array as $categoryId => $category) 
	{
		...
		{						
				...
				if($sub == 0) $class=" class=form_ac "; else $class="";
				$select="";	
				
				if( $categoryId == $text_grupa){$select='selected="selected"';}	
...
return $text;
	...
	function viewPrice($price, $kurs, $discount=0)
	...
	{	
...

		{
			...
			$price2 = round($price-$price*$discount,2);
			...
			if($kurs['position']==0)$price2=$kurs['icon']."&nbsp;".$price2;

			else $price2.=" ".$kurs['icon'];

			if($kurs['position']==0)$price=$kurs['icon']."&nbsp;".$price;

			else $price.=" ".$kurs['icon'];

			$price3[0]=$price2;

			$price3[1]="<span>$price</span>";
